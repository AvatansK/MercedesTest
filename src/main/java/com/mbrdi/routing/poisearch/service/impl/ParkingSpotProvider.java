...
import com.mbrdi.routing.exception.POISearchException;
import com.mbrdi.routing.geosearch.dtos.GeoPosition;
import com.mbrdi.routing.poisearch.dtos.POIFinderResponse;
import com.mbrdi.routing.poisearch.dtos.POILocationDetails;
import com.mbrdi.routing.poisearch.service.POIProvider;
...
@Service
@Slf4j
public class ParkingSpotProvider implements POIProvider {
  ...
  @Autowired
  ParkingSpotProvider(RestTemplate restTemplate,@Value("${here-map.discover-api}") String discoverApi,
      @Value("${here-map.api-key}")String accessToken,
      @Value("${here-map.parking-spot-category-type}")String categoryType){
  ...
  public CompletableFuture<List<POILocationDetails>> getPOILocationDetails(GeoPosition position){
    ...
    try{
      ...
      log.error("Exception in finding parking spot :: ",e);
      throw  new POISearchException("Exception while getting parking spots poi",e);
    }
